...
const testimonialsRow2: Testimonial[] = [
  {
    ...
    content:
      "The workflow automation features have saved our team countless hours. Best investment we've made this year!",
  },
  {
    name: "Marcus Chen",
    handle: "@mchen_dev",
...
const Testimonials = () => {
  const row1Items = testimonialsRow1.map((testimonial, index) => ({
    node: <TestimonialCard key={index} testimonial={testimonial} />,
    title: testimonial.name,
  }));

  const row2Items = testimonialsRow2.map((testimonial, index) => ({
    node: <TestimonialCard key={index} testimonial={testimonial} />,
    title: testimonial.name,
  }));
  ...
  return (
    <section className="flex flex-col items-center w-full py-16 md:py-26 overflow-hidden">
      ...
      >
        ...
        <h2 className={title({ size: "lg" })}>Loved by Thinkers</h2>
        <p
          className={subtitle({
