...
// Module-level state that persists across remounts (works with SSR)
...
const GooeyNav: React.FC<GooeyNavProps> = ({
	...
	// Use persisted state if available, otherwise use initialActiveIndex
	const [activeIndex, setActiveIndex] = useState<number>(() => {
		if (persistedActiveIndex !== null && hasInitialized) {
        ...
        `}
			...
			<div ref={containerRef} className="relative">
				...
				>
					<ul
						ref={navRef}
						className="flex gap-5 md:gap-8 list-none p-0 px-2 md:px-4 m-0 relative z-[3] text-sm md:text-base"
						style={{
							color: "white",
					...
					>
						{items.map((item, index) => (
							<li
								key={index}
								className={`rounded-full relative cursor-pointer transition-[background-color_color_box-shadow] duration-base shadow-[0_0_0.5px_1.5px_transparent] ${
									activeIndex === index ? "active" : "text-white"
								}`}
							>
								<Link
									className="outline-none py-[0.4em] px-[0.7em] md:py-[0.6em] md:px-[1em] inline-block"
									href={item.href}
